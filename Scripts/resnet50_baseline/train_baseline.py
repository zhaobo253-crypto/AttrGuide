...
# -*- coding: utf-8 -*-
"""
Train baseline breast ultrasound image classification model (ResNet50 only, no attribute guidance)

This script:
1) Creates dataloaders for train/val/test splits
2) Trains the baseline ResNet50 model with CrossEntropyLoss
"""
import argparse
import os
import random
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Baseline Breast Ultrasound Image Classification Model (ResNet50 only, 3 classes: benign, malignant, normal)")
...
def main():
    ...
    print("ResNet50 only, no attribute guidance")
    print("==========================================")
    ...
    backbone_feat_dim = 2048  # ResNet50 feature dimension
    print(f"Backbone feature dimension: {backbone_feat_dim} (ResNet50)")
