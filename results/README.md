...
This folder contains sanitized training logs for the public BUSI breast ViT-Base experiments and the DDTI thyroid ultrasound experiments.
...
The logs have been sanitized before release. Local server paths, usernames, scheduler directories, and offline WandB run identifiers have been replaced with placeholders such as `<PROJECT_DIR>`, `<THYROID_PROJECT_DIR>`, `<BREAST_DATA_ROOT>`, and `<PRETRAINED_WEIGHT>`.
