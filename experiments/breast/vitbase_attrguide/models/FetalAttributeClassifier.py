...
class FetalAttributeClassifier(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            backbone_type: Backbone ('resnet50', 'resnet101',  'vitbase')
            num_classes:  (3 for breast: benign, malignant, normal)
            num_attrs: 
            attr_emb_dim: 
            backbone_feat_dim: Backbone (ResNet50:2048, ResNet101:2048, ViT-Base:768)
            common_dim: 
            fusion_weight:  (0-1)
            resnet_path: backbone
            temperature: 
            grid_size: ViT
            dropout: Dropoutbaseline--dropout
        """
        ...
        # 3. dropout  baseline  args.dropout 
    ...
    def ClsBySimilar(self, x_vis_patches, attr_protos):
        ...
        Args:
            x_vis_patches: (batch_size, num_patches, common_dim) - patch
            attr_protos: (num_attrs, common_dim) - 
        
        Returns:
            attr_logits: (batch_size, num_attrs) - 
            patch_similarity: (batch_size, num_attrs, num_patches) - patch
        """
        ...
        patch_similarity = cosine_sim.permute(0, 2, 1)  # (B, num_attrs, N) - 
        
        # SIMPLIFIED: 
        # softmax
        similarity_weights = F.softmax(patch_similarity * 2.0, dim=2)  # (B, num_attrs, N)
        attr_logits = (patch_similarity * similarity_weights).sum(dim=2)  # (B, num_attrs) - 
    ...
    def attribute_to_class(self, attr_embed, patch_similarity=None):
        ...
        Args:
            attr_embed: (batch_size, num_attrs) - /
            patch_similarity: (batch_size, num_attrs, num_patches) - patch
        
        Returns:
            attr_cls_logits: (batch_size, num_classes) - 
        """
        ...
        # SIMPLIFIED: 
        attr_weights = attr_embed  # (B, num_attrs) - 
        ...
        # logitslogits
        attr_cls_logits = (attr_cls_logits - 0.5) * 20.0  # [-5, 5][-10, 10]
    ...
    def forward(self, x, attrs_matrix):
        ...
        Args:
            x: (batch_size, 3, H, W) - 
            attrs_matrix: (num_attrs, attr_emb_dim) - 
        ...
        if len(x_vis_raw.shape) == 4:
            # ResNet 4D: (batch_size, channels, H, W)
            ...
            # ViT 3D: (batch_size, num_patches+1, backbone_feat_dim)
            x_vis = x_vis_raw[:, 0, :]  # (B, C) - [CLS] token
            x_vis_patches_raw = x_vis_raw[:, 1:, :]  # (B, N, C) - patch tokens
        else:
            # ResNet 2D: (batch_size, backbone_feat_dim)
            x_vis = x_vis_raw
        ...
        # CRITICAL: patch_similarity
        # patch_similarity
        result['patch_similarity'] = patch_similarity  # (B, num_attrs, N) - 
