...
DATA_ROOT="${DATA_ROOT:-${REPO_ROOT}/data/breast}"
ATTR_CSV="${ATTR_CSV:-${REPO_ROOT}/examples/attributes_breast.csv}"
ATTR_EMB_PATH="${ATTR_EMB_PATH:-${REPO_ROOT}/data/attribute_embeddings_breast.pt}"
BACKBONE_WEIGHT="${BACKBONE_WEIGHT:-${REPO_ROOT}/pretrained/vit_b_16-c867db91.pth}"
SAVE_DIR="${SAVE_DIR:-${REPO_ROOT}/outputs/breast_vitbase_attrguide}"
...
if [ ! -f "${BACKBONE_WEIGHT}" ]; then echo "BACKBONE_WEIGHT not found: ${BACKBONE_WEIGHT}" >&2; echo "Set BACKBONE_WEIGHT=/path/to/vit_b_16-c867db91.pth and rerun this script." >&2; exit 1; fi
...
python train_fetal_attribute_model.py \
  ...
  --lr "${LR:-0.00007}" \
  --weight_decay "${WEIGHT_DECAY:-6e-4}" \
